...
5. recovery code는 TOTP 재등록용 1회 복구 키다. `POST /api/v1/admin/security/mfa/recover` 성공 후 같은 세션에서 `POST /api/v1/admin/security/mfa/recovery/enroll`로 새 TOTP secret을 발급받고, `/confirm`으로 새 TOTP code를 확정한다.
6. recovery 세션은 고위험 approve/reject에 사용할 수 없다. recovery 세션으로는 TOTP 재등록만 수행하고, 고위험 결정 전에는 반드시 `/security/mfa/verify`로 TOTP 세션을 다시 만든다.
7. dev smoke에서 여러 Super Admin 토큰을 동시에 쓸 때는 `MINT_SESSION_ID`를 사용자별로 다르게 지정한다. 동일한 `sessionId`를 재사용하면 MFA 세션이 서로 덮어써져 recovery/TOTP 검증 결과가 섞일 수 있다.
...
| `ADMIN_MFA_RECOVERY_REQUIRED` | recovery 세션 없이 TOTP 재등록 시도 | recovery code로 `/security/mfa/recover`부터 수행 |
...
| 관리자 MFA | `admin_mfa.enroll` / `.confirm` / `.verify` / `.recover` / `.recovery_enroll` |
...
배포 smoke에서 다중 Super Admin MFA를 확인할 때:

```powershell
$env:MINT_ROLE = "R-SUPER-ADMIN"
$env:MINT_USER_ID = "smoke-super-1"
$env:MINT_SESSION_ID = "smoke-super-1"
python scripts/mint_dev_admin_token.py

$env:MINT_USER_ID = "smoke-super-2"
$env:MINT_SESSION_ID = "smoke-super-2"
python scripts/mint_dev_admin_token.py
```

Recovery code smoke는 `/api/v1/admin/security/mfa/recover` 성공 후 `recoveryCodesRemaining`이 1 감소하는지, 같은 code 재사용이 거부되는지, 같은 세션에서 `/api/v1/admin/security/mfa/recovery/enroll`과 `/confirm`으로 새 TOTP/recovery code 세트가 발급되는지 확인한다. recovery 세션으로 고위험 approve/reject가 `ADMIN_MFA_TOTP_REQUIRED`로 막히는지도 확인한다. 이후 `/security/mfa/verify`로 TOTP 세션을 다시 만든 뒤 동일 고위험 결정이 성공하는지 재확인한다.
